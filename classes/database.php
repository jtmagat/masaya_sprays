...
$pdo = $this->connection;
    try {
        ...
        // 1) calculate cart subtotal (your existing method)
        $cartSummary = $this->getCheckoutSummary($_SESSION['selected_cart_items'] ?? null);
        $subtotal = floatval($cartSummary['total']);

        // 2) shipping fee from $data (should be passed from form)
        $shipping_fee = isset($data['shipping_fee']) ? floatval($data['shipping_fee']) : 0.00;

        // 3) compute total
        $total_amount = $subtotal + $shipping_fee;

        // 4) insert into orders table (adjust column names to match your DB)
        $stmt = $pdo->prepare("INSERT INTO orders 
            (customer_id, payment_method, total_amount, shipping_fee, order_status, o_created_at)
            VALUES (?, ?, ?, ?, 'Pending', NOW())");

        $customer_id = $this->getCurrentCustomerId(); // adapt to your class
        $payment_method = $data['payment_method'] ?? 'Cash on Delivery';

        $stmt->execute([$customer_id, $payment_method, $total_amount, $shipping_fee]);
        $orderId = $pdo->lastInsertId();

        // 5) insert order_items (example)
        ...
        $pdo->rollBack();
        throw $e; // let the caller handle error
    }
}
